...
export default class TestAsyncStoreAllProduct extends Component{
  constructor() {
    super();
    this.state = {
      listProducts: [],
  ...
  onPressStart() {
    ...
    if (!isLoad) {
      ...
      .then((response) => {
        ...
        if (data.length > 0) {
          AsyncStorage.setItem('@PRODUCTS', JSON.stringify(data), (res) => {
            this.setState({
              startTime: (new Date()).getTime(),
              mode: 3
            });
            // STEP 3 : get data from AsyncStorage 
            AsyncStorage.getItem('@PRODUCTS', (err, stores) => {
              this.setState({
                listProducts: JSON.parse(stores),
                isLoad: false,
                startTime: 0,
                mode: 0
              });
